---
description: Automated, always-on anti-vamp detection that protects every live token's identity.
---

# Smart <span class="g">Anti-Vamp Protection</span>
...
* **Blocked at the door.** A flagged clone is **stopped at deployment**, it never reaches the market, so there's no window where an impostor trades alongside the original.
* **Immediate protection.** A token's identity is protected **from the moment it launches**, so there's no early gap for a clone to slip in before the original gains traction.
...
Anti-vamp protection guards a token's *identity*. The rest of the trust picture, launch guardrails and dev-buy disclosure, lives on the token page: see [transparency tags](../terminal/transparency-tags.md).
